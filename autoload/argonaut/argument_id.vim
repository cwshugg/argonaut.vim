...
function! argonaut#argument_id#new() abort
    " make a copy of the template object
    return deepcopy(s:argument_id_template)
endfunction

" Checks the given object for all fields in the argument identifier template.
" An error is thrown if they are all not found.
function! argonaut#argument_id#verify(aid) abort
    for s:key in keys(s:argument_id_template)
        if !has_key(a:aid, s:key)
            call argonaut#utils#panic('the provided object does not appear to be a valid argument identifier object')
        endif
    endfor
endfunction

" Builds and returns a string representation of the argument ID object.
function! argonaut#argument_id#to_string(aid) abort
    call argonaut#argument_id#verify(a:aid)
    return '' . a:aid.prefix . a:aid.name
endfunction

" Setter for `name`.
function! argonaut#argument_id#set_name(aid, name) abort
    call argonaut#argument_id#verify(a:aid)
    
    " make sure the name isn't null
    let s:name = argonaut#utils#sanitize_value(a:name)
    if argonaut#utils#is_null(s:name)
        call argonaut#utils#panic('an argument identifier cannot have a null name')
    endif
...
" Getter for `name`.
function! argonaut#argument_id#get_name(aid) abort
    call argonaut#argument_id#verify(a:aid)
    return get(a:aid, 'name')
endfunction

" Setter for `prefix`.
function! argonaut#argument_id#set_prefix(aid, prefix) abort
    call argonaut#argument_id#verify(a:aid)
    
    " make sure the prefix isn't null
    let s:prefix = argonaut#utils#sanitize_value(a:prefix)
    if argonaut#utils#is_null(s:prefix)
        call argonaut#utils#panic('an argument identifier cannot have a null prefix')
    endif
...
" Getter for `prefix`.
function! argonaut#argument_id#get_prefix(aid) abort
    call argonaut#argument_id#verify(a:aid)
    return get(a:aid, 'prefix')
endfunction

" Setter for `case_sensitive`.
function! argonaut#argument_id#set_case_sensitive(aid, case_sensitive) abort
    call argonaut#argument_id#verify(a:aid)
    let a:aid.case_sensitive = argonaut#utils#sanitize_bool(a:case_sensitive)
endfunction

" Getter for `case_sensitive`.
function! argonaut#argument_id#get_case_sensitive(aid) abort
    call argonaut#argument_id#verify(a:aid)
...
function! argonaut#argument_id#cmp(aid, str) abort
    call argonaut#argument_id#verify(a:aid)
    
    " build a string with the argument ID to compare with
    let s:aid_str = argonaut#argument_id#to_string(a:aid)
